...
class CMAEsPopulation:
    def __init__(self, pop_size, dim, sigma=1.0, lr=.5):
        self.sigma = sigma
        self.lr = lr
        self.dim = dim
    ...
    def __init_population(self):
        ...
        self.population = np.random.multivariate_normal(self.mean, self.sigma**2 * np.eye(dim), self.pop_size)

    def run_pop(self, obj_func):
        return sorted([(i, obj_func(i)) for i in self.population], key=lambda x: x[1])
    
    def update_sigma(self, mean):
        diff = self.population - mean
        cov_mat = np.cov(diff, rowvar=False)
        self.sigma = (1 - self.lr) * self.sigma + self.lr * cov_mat
