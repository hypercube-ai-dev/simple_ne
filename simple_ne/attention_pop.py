...
class SimpleNEPopulation():
    ...
    def create_genome(self):
        ...
        return AttentionNeNet(nodes, self.in_size, self.out_size)
    
    def mutate_genome(self, net: AttentionNeNet):
        mutated = False
        ...
        if add_node == True:
            ...
            key = len(net_nodes)
            net_nodes.append(AttentionNeNode(
                activations[activation_key],
                weights,
                connection_keys,
                key
        ...
        if mutated == True:
            return AttentionNeNet(net_nodes, self.in_size, self.out_size)
        else:
            return None
